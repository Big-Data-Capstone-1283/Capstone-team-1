    ...
    def dateTimeGenerationTaperPoints(companyName:String, companyStartMS:Long, companyEndMS:Long, companyTaperPoints:Array[Long],
      ...
      */
      ...
      while(i < companyEndMS){
        if(i > companyTaperPoints(taperTracker)){
        ...
        var firstWeight = (i - companyTaperPoints(taperTracker)) / (companyTaperPoints(taperTracker + 1) - companyTaperPoints(taperTracker))
        var generationAmount = (
          (companySalesRates(taperTracker)*firstWeight) + (companySalesRates(taperTracker + 1)*(1 - firstWeight)) +
          rand.between(
            -((companySalesVariation(taperTracker)*firstWeight) + companySalesVariation(taperTracker + 1)*(1 - firstWeight)),
            ((companySalesVariation(taperTracker)*firstWeight) + companySalesVariation(taperTracker + 1)*(1 - firstWeight))
                      )
          ).toInt
      ...
      val returnArray = randomArrayArrayBuffer.flatten.map(Long => Timestamp).toArray
      return returnArray
    }
